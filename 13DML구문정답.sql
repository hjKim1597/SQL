...
----------------------------------------------------------------------------------------------------
--DELETE구문
--트랜잭션이 있긴하지만, 삭제하기전에 반드시 SELECT문으로 삭제 조건에 해당하는 데이터를 꼭 확인하는 습관을 들이자
...
DELETE FROM DEPARTMENTS WHERE DEPARTMENT_ID = 100; -- EMPLOYEES에서 100번 데이터를 FK로 사용하고 있어서, 지울수 없다.
...
SELECT * FROM EMPS;
-- 2ND - 서브쿼리절로 다른테이블을 가져오는게 아니라, 직접 값을 넣을 때 DUAL을 쓸수도 있습니다.
MERGE INTO EMPS A
USING DUAL
...
DELETE FROM DEPTS WHERE DEPARTMENT_ID = 320; -- (SELECT DEPARTMENT_ID FROM DEPTS WHERE DEPARTMENT_NAME = '영업')
--2. 부서명 NOC를 삭제하세요
