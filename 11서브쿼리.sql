...
-- 주의할점 - 여러행이 나오는 구문이라면, 다중행 서브쿼리 연산자를 써줘야합니다. (다중행 서브쿼리로 처리)
...
FROM EMPLOYEES E;
-- 한번에 하나의 컬럼을 가져오기 때문에, 많은 열을 가지고 올때는 오히려 JOIN구문이 가독성이 좋을 수 있습니다.
SELECT FIRST_NAME,
       JOB_ID,
       (SELECT JOB_TITLE FROM JOBS J WHERE J.JOB_ID = E.JOB_ID) AS JOB_TITLE,
...
WHERE ROWNUM BETWEEN 11 AND 20; -- ROWNUM은 특징이 반드시 1부터 시작해야함. (결과가 X)

-- ORDER를 먼저 시킬 결과를 만들고, ROWNUM 가상열로 다시만들고, 재조회
SELECT *
FROM (
      SELECT ROWNUM AS RN, --가상열(인위적으로 생성된열)
             FIRST_NAME,
             SALARY
      FROM (
            SELECT FIRST_NAME,
