...
SELECT DEPARTMENT_ID,
        ...
        COUNT(*) AS 부서인원수 ,
        COUNT(*) OVER() 전체카운트 --COUNT (*) OVER()를 사용하면, 총행의 갯수를 출력가능
FROM EMPLOYEES
...
AVG(SALARY),
COUNT(*),
COUNT(COMMISSION_PCT) AS 커미션받는사람
FROM EMPLOYEES
...
--
--부서아이디가 NULL이 아닌 데이터중에서, 입사일은 05년도인 사람들의 급여평균, 급여합을 구하고
--평균급여는 5000이상인 데이터만, 부서아이디로 내림차순 정렬
SELECT DEPARTMENT_ID, 
AVG(SALARY),
SUM(SALARY)
FROM EMPLOYEES
...
--------------------------------------------------------------------
--시험대비용
--ROLLUP - GROUP BY 절과 함께 사용되고, 상위그룹에 합계, 토탈 등을 구함
SELECT DEPARTMENT_ID,
    SUM(SALARY),
    AVG(SALARY)
    FROM EMPLOYEES
...
--GROUPING 함수: 그룹바이로 만들어진 경우는 0반환, 롤업 또는 큐브로 만들어진 행의 경우 1 반환
...
SELECT TO_CHAR(HIRE_DATE, 'YY'), --연도별
        COUNT(*)
FROM EMPLOYEES
...
SELECT DECODE(GROUPING(DEPARTMENT_ID), 1, '합계', DEPARTMENT_ID) AS DEPARTMENT_ID,
        DECODE(GROUPING(JOB_ID), 0, '소계', JOB_ID) AS JOB_ID,
