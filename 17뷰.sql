-- 뷰
--뷰는 제한적인 데이터를 쉽게 보기 위해서 미리 만들어 놓은 가상 테이블(가짜)
--자주 사용되는 컬럼을 저장하면, 관리가 용이함
--뷰는 물리적으로 데이터가 저장된것은 아니고, 원본 테이블을 기반으로 한 가상테이블 이라고 생각하면 됨
--뷰를 만들려면 권한이 필요함(HR은 권한을 가지고 있음)
...
--단순뷰는 뷰를 통해서 INSERT나 UPDATE가 가능한데, 제약사항들이 있음
...
--뷰의 옵션
--WITH CHECK OPTION (WHERE 절에 있는 컬럼의 변경을 금지함)
