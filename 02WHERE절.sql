...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE >= '06/03/10';
...
--LIKE 연산자 - 검색에 사용됨, 리터럴 문자 %(무엇이든올수있다) _ (자릿수)
...
SELECT * FROM EMPLOYEES WHERE FIRST_NAME LIKE '__a%';-- 3번째 자리가 a인 데이터
...
--AND 먼저 실행 //OR 먼저 실행 위해서 () 사용
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR 'JOB_ID' = 'FI_MGR' AND SALARY >= 6000; 
SELECT * FROM EMPLOYEES WHERE (JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR') AND SALARY >= 6000;

--NOR 부정의 의미 - 연산키워드와 사용됨
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID NOT IN (50, 60); -- 부서아이디가 50, 60 이 아닌
...
SELECT FIRST_NAME, SALARY * 12 AS 연봉 FROM  EMPLOYEES ORDER BY 연봉; -- 별칭을 ORDER절 에서 사용할 수 있음
