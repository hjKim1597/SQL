...
SELECT INFO.ID,
       INFO.TITLE,
        INFO.CONTENT,
        INFO.AUTH_ID, -- AUTH_ID는 양측에 다있는 KEY, 테이블.컬럼명 기입해줘야함
        AUTH.NAME
FROM INFO 
INNER JOIN AUTH
ON INFO.AUTH_ID = AUTH.AUTH_ID;
...
-------------------------------------------------------------------------
--OUTER JOIN
--LEFT OUTER JOIN(OUTER 생략가능) - 왼쪽 테이블이 기준이 돼서, 왼쪽 테이블은 다나옴
SELECT * FROM INFO I LEFT OUTER JOIN AUTH A ON I.AUTH_ID = A.AUTH_ID;

--RIGHT OUTER JOIN - 오른쪽 테이블이 기준이 돼서, 오른쪽 테이블은 다나옴
...
--CROSS JOIN(잘못된 조인의 형태- 실제 쓸일은 없음)
SELECT * FROM INFO I CROSS JOIN AUTH A;

-------------------------------------------------------------------------------
--SELF JOIN (하나의 테이블을 가지고 조인을 거는 것 - 조건 테이블 안에 연결가능한 키 필요함)
...
-------------------------------------------------------------------------------
--오라클 조인: 오라클에서만 사용할 수 있고, 조인할 테이블을 FROM에 씀, 조인조건을 WHERE에 씀
...
--평범하게 생각하면 N테이블에 1 테이블 붙이는 게 가장 많다
